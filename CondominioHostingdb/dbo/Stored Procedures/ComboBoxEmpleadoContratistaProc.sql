...
create proc ComboBoxEmpleadoContratistaProc
@PersonaId int,
@PersonaTipoId int,
@CondominioId int
as
if(@PersonaTipoId = 4)
	begin
		...
		WHERE        (dbo.PersonaTable.PersonaActivo = 1) AND (dbo.PersonaTable.PersonaTipoId = 5 OR
								 dbo.PersonaTable.PersonaTipoId = 3) AND (c.CondominioPropietarioId = @PersonaId)
	end
else
	begin
		SELECT        dbo.PersonaTable.PersonaId, dbo.PersonaTable.PersonaNombre, dbo.PersonaTable.PersonaApellido, dbo.PersonaTable.PersonaDocumento, dbo.PersonaTable.PersonaTelefono, 
		...
		FROM            dbo.PersonaTable INNER JOIN
								 ...
								 left join CondominioTable c on PersonaTable.PersonaCondominioId = c.CondominioId
		WHERE        (dbo.PersonaTable.PersonaActivo = 1) AND (dbo.PersonaTable.PersonaTipoId = 5 OR
								 dbo.PersonaTable.PersonaTipoId = 3) AND (c.CondominioId = @CondominioId)
	end